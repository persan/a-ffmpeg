...
package FFMpeg.opt is
   ...
   --  arg-macro: function set_int_list (obj, name, val, term, flags)
   --    return av_int_list_length(val, term) > INT_MAX / sizeof(*(val)) ? AVERROR(EINVAL) : set_bin(obj, name, (const uint8_t *)(val), av_int_list_length(val, term) * sizeof(*(val)), flags);
  ...
  -- * is set to the correct value. Then, set_defaults() can be called to
  -- * initialize defaults. After that the struct is ready to be used with the
  -- * AVOptions API.
  -- *
  -- * When cleaning up, you may use the free() function to automatically
  ...
  -- * The basic functions for examining options are next(), which iterates
  -- * over all options defined for one object, and find(), which searches
  -- * for an option with the given name.
  -- *
  -- * The situation is more complicated with nesting. An AVOptions-enabled struct
  -- * may have AVOptions-enabled children. Passing the AV_OPT_SEARCH_CHILDREN flag
  -- * to find() will make the function search children recursively.
  -- *
  -- * For enumerating there are basically two cases. The first is when you want to
  -- * get all options that may potentially exist on the struct and its children
  -- * (e.g.  when constructing documentation). In that case you should call
  -- * child_class_next() recursively on the parent struct's AVClass.  The
  -- * second case is when you have an already initialized struct with all its
  -- * children and you want to get all options that can be actually written or read
  -- * from it. In that case you should call child_next() recursively (and
  -- * next() on each result).
  -- *
  -- * @subsection avoptions_use_get_set Reading and writing AVOptions
  -- * When setting options, you often have a string read directly from the
  -- * user. In such a case, simply passing it to set() is enough. For
  -- * non-string type options, set() will parse the string according to the
  -- * option type.
  -- *
  -- * Similarly get() will read any option type and convert it to a string
  -- * which will be returned. Do not forget that the string is allocated, so you
  -- * have to free it with av_free().
  -- *
  -- * In some cases it may be more convenient to put all options into an
  -- * AVDictionary and call set_dict() on it. A specific case of this
  ...
  --     *       provide AV_OPT_MULTI_COMPONENT_RANGE to query_ranges or
  --     *       query_ranges_default function.
  --     *
  ...
  --     * query_ranges(&ranges, obj, key, AV_OPT_MULTI_COMPONENT_RANGE);
  ...
  --
   ...
   procedure set_defaults (s : System.Address);  -- /usr/include/ffmpeg/libavutil/opt.h:388
  ...
  -- * the error code issued by set() if a key/value pair
  -- * cannot be set
  --
  ...
  --
   ...
   procedure free (obj : System.Address);  -- /usr/include/ffmpeg/libavutil/opt.h:454
  ...
  --
   ...
   function set_dict (obj : System.Address; options : System.Address) return int;  -- /usr/include/ffmpeg/libavutil/opt.h:480
  ...
  -- * and get numbers out of them. They do the same thing as set(),
  ...
  --*
  -- *  The obj passed to find() is fake -- only a double pointer to AVClass
  ...
  --*
  -- *  In get, return NULL if the option has a pointer type and is set to NULL,
  -- *  rather than returning an empty string.
  --

  --*
  -- *  Allows query_ranges and query_ranges_default to return more than
  ...
  -- * directly with set(). Use special calls which take an options
  ...
  -- * @param prev result of the previous call to next() on this object
  -- *             or NULL
  -- * @return next AVOption or NULL
  --

   function next (obj : System.Address; prev : access constant AVOption) return access constant AVOption;  -- /usr/include/ffmpeg/libavutil/opt.h:638
  ...
  --
   ...
   function child_next (obj : System.Address; prev : System.Address) return System.Address;  -- /usr/include/ffmpeg/libavutil/opt.h:646
  ...
  --
   ...
   function child_class_next (parent : access constant FFMpeg.Low_Level.ffmpeg_libavutil_log_h.AVClass; prev : access constant FFMpeg.Low_Level.ffmpeg_libavutil_log_h.AVClass) return access constant FFMpeg.Low_Level.ffmpeg_libavutil_log_h.AVClass;  -- /usr/include/ffmpeg/libavutil/opt.h:654
  ...
  -- * @param[in] val The value to set. In case of set() if the field is not
  ...
  -- * @param search_flags flags passed to find2. I.e. if AV_OPT_SEARCH_CHILDREN
  ...
  -- * @param[in] search_flags flags passed to find2. I.e. if AV_OPT_SEARCH_CHILDREN
  ...
  -- *
  -- * @note if AV_OPT_ALLOW_NULL is set in search_flags in get, and the option has
  ...
  --
   ...
   procedure freep_ranges (ranges : System.Address);  -- /usr/include/ffmpeg/libavutil/opt.h:767
  ...
  -- *
  -- * The result must be freed with freep_ranges.
  -- *
  ...
  --
   ...
   function copy (dest : System.Address; src : System.Address) return int;  -- /usr/include/ffmpeg/libavutil/opt.h:794
  ...
  -- *              AV_OPT_MULTI_COMPONENT_RANGE indicates that function may return more than one component, @see AVOptionRanges
  -- *
  -- * The result must be freed with free_ranges.
  -- *
  -- * @return number of compontents returned on success, a negative errro code otherwise
  --
  ...
  --
   ...
   function is_set_to_default (obj : System.Address; o : access constant AVOption) return int;  -- /usr/include/ffmpeg/libavutil/opt.h:824
  ...
  -- * Such string may be passed back to set_from_string() in order to restore option values.
